  ...
  apriltag_msgs__msg__AprilTagDetection__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    apriltag_msgs__msg__AprilTagDetection * data =
      (apriltag_msgs__msg__AprilTagDetection *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!apriltag_msgs__msg__AprilTagDetection__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          apriltag_msgs__msg__AprilTagDetection__fini(&data[i]);
        }
        free(data);
        return false;
      }
