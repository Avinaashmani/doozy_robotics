  ...
  apriltag_msgs__msg__AprilTagDetectionArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    apriltag_msgs__msg__AprilTagDetectionArray * data =
      (apriltag_msgs__msg__AprilTagDetectionArray *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!apriltag_msgs__msg__AprilTagDetectionArray__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          apriltag_msgs__msg__AprilTagDetectionArray__fini(&data[i]);
        }
        free(data);
        return false;
      }
